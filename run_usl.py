...
class HiddenParameterEnv:
    ...
    def __init__(self, env_id: str, device: str, env_config: EnvironmentConfig, num_envs: int = 1, render_mode: Optional[str] = None, **kwargs) -> None:
        ...
        self.hidden_parameters_dim = self._initial_info['hidden_parameters_dim']
        self._device = torch.device(device)
        self._count = 0
        self._num_envs = num_envs
        self.obs_dims = {k: v for k, v in self._initial_info['obs_dims'].items() if not k.startswith('_')}
        self.slices = self.get_slices()

        self._org_observation_space = self.env.observation_space
    ...
    def augment_state(self, obs: np.ndarray, info: np.ndarray) -> np.ndarray:
        ...
        hidden_info = info['hidden_parameters_features'] if self.oracle else np.zeros(self.hidden_parameters_dim)
...
if __name__ == "__main__":
    ...
    if args.eval_only:
        ...
        load_model_dir = f'./logs/oracle/1_USL_{load_env_info}_v1-seed{args.seed}-0'
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if (t + 1) % args.eval_freq == 0:
            ...
            policy.save(f"./{logger.log_dir}/usl_model")